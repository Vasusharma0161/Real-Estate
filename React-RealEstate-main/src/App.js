import Nav from './components/Nav.js';
import Footer from './components/Footer.js';
import Navbar from './components/Navbar.js';
import Job from './components/Job.js';
import About from './About';
import SearchInput from './components/SearchInput.js';
import Home from './Home'
import React, { Component } from 'react';
import axios from 'axios'
import { Switch, Route, Redirect, useHistory, useParams } from "react-router-dom";
let placeImg = [];
let placeTitle = [];
let contact = [];
let placeLocation = [];
let price = [];
let placeLink = [];
let AllData = async () => {
    axios("https://realestate-info.herokuapp.com/home", { mode: "no-cors" }).then(response => { placeImg = [...response.data.img]; placeTitle = [...response.data.title];contact = [...response.data.contact]; placeLocation = [...response.data.location]; price = [...response.data.price]; placeLink = [...response.data.Link]; console.log(response.data); });
};
AllData()

class App extends Component {
    render() {
        return (
            <>
                <div className="z-30">
                <Nav img="data:image/png;base64,"
                    name="VardhMan Estate" navLink={["Home", "Location", "Pricing","Contact", "i"]} />
                <Navbar img="data:image/png;base64,"
                    name="VardhMan Estate" navLink={["Home", "Location", "Pricing","Contact", "i"]} />
                </div>
                    <Switch>
                    <Route exact path="/" component={Home} />
                    <Route path="/home" render={() => (
                        <div className="text-center my-44 lg:px-52 md:px-44 sm:px-20 px-10 overflow-hidden">
                            <div className="flex justify-center mx-auto"><p className="uppercase max-w-max mx-auto p-2 rounded-full w-max px-4 text-center mx-auto xl:text-6xl lg:text-4xl md:text-xl text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-gray-100 to-black to-transparent to-blue-500">Home</p></div>
                                  {placeTitle.map((title,i) => (
                                      <Job key={i} link={placeLink[i]} location={placeLocation[i]} img={placeImg[i]} title={placeTitle[i]} price={"Rs. "+price[i]} description={contact[i]}/>
                                ))}
                            </div>
                        )}
                        exact={true} />
                    <Route path="/contact" render={() => (
                        <div className="text-center my-44 lg:px-52 md:px-44 sm:px-20 px-10 overflow-hidden">
                            <div className="flex justify-center mx-auto"><p className="uppercase max-w-max mx-auto p-2 rounded-full w-max px-4 text-center mx-auto xl:text-6xl lg:text-4xl md:text-xl text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-gray-100 to-black to-transparent to-blue-500">Contact</p></div>
                                  {placeTitle.map((title,i) => (
                                      <Job key={i} link={"tel:" + contact[i]} location={contact[i]} img={placeImg[i]}/>
                                ))}
                            </div>
                        )}
                        exact={true} />
                    <Route path="/about" render={() => (
                        <div className="text-center my-44 lg:px-52 md:px-44 sm:px-20 px-10 overflow-hidden">
                            <div className="flex justify-center mx-auto"><p className="uppercase max-w-max mx-auto p-2 rounded-full w-max px-4 text-center mx-auto xl:text-6xl lg:text-4xl md:text-xl text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-gray-100 to-black to-transparent to-blue-500">About</p></div>
                                  <About/>
                            </div>
                        )}
                        exact={true} />
                    <Route path="/location" render={() => (
                        <div className="text-center my-44 lg:px-52 md:px-44 sm:px-20 px-10 overflow-hidden">
                            <div className="uppercase flex justify-center mx-auto"><p className="max-w-max mx-auto p-2 rounded-full w-max px-4 text-center mx-auto xl:text-6xl lg:text-4xl md:text-xl text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-gray-100 to-black to-transparent to-blue-500">Location</p></div>
                                  {placeTitle.map((title,i) => (
                                      <Job key={i} link={placeLink[i]} img={placeImg[i]} location={placeLocation[i]} />
                                ))}
                            </div>
                        )}
                        exact={true} />
                    <Route path="/price" render={() => (
                        <div className="text-center my-44 lg:px-52 md:px-44 sm:px-20 px-10 overflow-hidden">
                            <div className="flex justify-center mx-auto"><p className="max-w-max mx-auto p-2 rounded-full w-max px-4 text-center mx-auto xl:text-6xl lg:text-4xl md:text-xl text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-gray-100 to-black to-transparent to-blue-500">Pricing</p></div>
                                  {placeTitle.map((title,i) => (
                                      <Job key={i} link={placeLink[i]} location={'Rs. '+price[i]} img={placeImg[i]}/>
                                ))}
                            </div>
                        )}
                        exact={true} />
                    <Redirect to="/"/>
                </Switch>
                <Footer/>
            </>
        );
    }
}

export default App;